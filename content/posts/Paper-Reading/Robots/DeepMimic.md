---
title: "DeepMimic: 从动作捕捉数据学习物理仿真角色技能"
date: "2026-04-10"
description: "SIGGRAPH 2018 论文 DeepMimic:把动作捕捉数据作为强约束奖励,结合 PPO + RSI + ET 训练物理仿真角色完成后空翻、回旋踢等高难度动作。"
...
![image-20251231205238511](./assets/image-20251231205238511.png)
...
![image-20251231210006240](./assets/image-20251231210006240.png)

* 论文中提出的框架可以为多种角色提供策略训练

![image-20251231210136161](./assets/image-20251231210136161.png)
...
* 双网络架构(Actor-Critic)：系统里有两个神经网络在同时训练 ：
  ...
  * 价值网络 ($V$, Value): 也就是“评委”，负责打分，告诉演员当前状态好不好。
    ...
    * 输出：单一标量 (Single Scalar):  代表 $V(s)$，即预测“站在当前这个状态下，直到游戏结束，我总共能拿到多少分”。如果这个分很高，说明当前姿势很有前途；如果分很低，说明快要摔倒了。
...
* 原理： 通常我们计算奖励是对比当前动作和*某一个*参考片段。但在这种模式下，系统会同时拿当前动作去对比*这一组*参考片段（比如 5 个不同的走路片段），然后取最高分作为最终奖励 。
  * $$r_t = \max_{j} r_t^j$$
...
* 训练： 训练时随机切换这个指令，强迫网络学会：不仅要会做动作，还要学会在动作之间平滑过渡（Transition） 。
