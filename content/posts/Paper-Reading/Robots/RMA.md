...
date: "2026-04-13"
description: "RSS 2021 论文 RMA:两阶段训练让四足机器人在线适应真实世界的不可预测物理(打滑、负载变化、电机磨损),实现毫秒级的快速运动自适应。"
tags: ["论文阅读","强化学习","四足机器人","域随机化"]
category: "论文阅读"
author: "zhuzichao"
draft: false
---
...
* 目标：让机器人像动物一样，在无需人工干预的情况下，==毫秒级==适应新环境。实际上，也就是需要机器人具备实时适应（Online Adaptation）的能力。
...
* Inputs
  ...
  * 历史信息序列: 过去一段时间（约 0.5秒，k=50步）的状态 ($x_{t-k:t-1}$) 和 动作 ($a_{t-k:t-1}$) 。

  - *注意：此时不再输入特权信息 $e_t$。*

* 核心组件与过程 (Process):

  * 适应模块 ($\phi$): 一个基于 1-D CNN（一维卷积神经网络）的模型，擅长处理时间序列数据。
...
​	这是将模型应用到真实的 Unitree A1 机器人上。此时环境编码器 ($\mu$) 被丢弃，只保留策略 ($\pi$) 和适应模块 ($\phi$ )。

- 输入 (Inputs):
  - 仅依赖机器人的实时状态 ($x_t$) 和 历史状态/动作序列。
  - *现实世界中没有 $e_t$（特权信息）。*
